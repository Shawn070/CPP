...
如果要使用一个文件输入流（ifstream），必要再构造函数中或者使用open 函数把该流与一个特定磁盘文件关联起来。

ios_base::in | 打开文件用于输入（默认）
----|----
ios_base:binary | 以而二进制模式（默认模式是文本模式）打开文件

2. 输入流的 close 函数

3. get 函数

非格式化 get 函数的功能与提取运算符（“>>”）很相像，主要的不同点是 get 函数再读入数据时包括空白字符，而提取运算符在默认情况下拒绝接受空白字符。参见 例11-7.cpp

4. getline 函数

istream 类具有成员函数 getline，允许从输入流中读取多个字符，并且允许指定输入终止字符（默认换行符）。将输入结果存在字符数组中，大小不能自动扩展。非成员函数 getline 有相同功能（声明在 string 头文件中），但可以将结果保存在 string 类型的对象中。可接受两个参数，前两个表示输入流和保存结果的string 对象，第三个参数可选，表示终止字符。参见 例11-8.cpp

5. read 函数

从一个文件读字节到一个指定的存储器区域，由长度参数确定要读的字节数。参见 例11-9.cpp
